...
@RestController
@RequestMapping("/api/letterdecrypt")
public class LetterDecryptorApiController {
    ...
    /*
     * @GetMapping("/{expression}")
     * public ResponseEntity<JsonNode> getIsLeapYear(@PathVariable String
     * expression) throws JsonMappingException, JsonProcessingException {
     * // Backend Year Object
     * Calculator calculator_obj = new Calculator(expression);
     * 
     * 
     ...
     * JsonNode json = mapper.readTree(calculator_obj.toStringJson()); // this
    ...
    @GetMapping("all/{text}")
    public ResponseEntity<JsonNode> decryptText(@PathVariable String text)
